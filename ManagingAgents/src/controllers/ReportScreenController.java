...
public class ReportScreenController implements Initializable  {
	...
	private ObservableList<Sales_Report> saleList = null;
	
	@FXML
	private MFXTableView<Sales_Report> table;
;
public ReportScreenController() {
	
}
public ReportScreenController(boolean action) {
	noAction = action;
}
...
public void initialize(URL arg0, ResourceBundle arg1) {
	...
	public void loadTable() {
		saleList = DAO_Sales_Report.getInstance().selectAll2();
	...
	public void reloadTable() {
		...
		loadTable();
		 ...
		 public void setupTable() {
				MFXTableColumn<Sales_Report> idColumn = new MFXTableColumn<>("Id", true, Comparator.comparing(Sales_Report::getSalesReport_Id));
				MFXTableColumn<Sales_Report> dateColumn = new MFXTableColumn<>("Date", true, Comparator.comparing(Sales_Report::getSalesReport_Date));
				
				

				// Set row cell factories
				idColumn.setRowCellFactory(col -> new MFXTableRowCell<>(Sales_Report::getSalesReport_Id));
				dateColumn.setRowCellFactory(col -> new MFXTableRowCell<>(Sales_Report::getSalesReport_Date));
				...
				table.getFilters().add(new IntegerFilter<>("Id", Sales_Report::getSalesReport_Id));
				table.getFilters().add(new IntegerFilter<>("Month", Sales_Report::getMonth));
				table.getFilters().add(new IntegerFilter<>("Year", Sales_Report::getYear));
				...
				table.getSelectionModel().selectionProperty().addListener((observable, oldValue, newValue) -> {
					if (newValue != null) {
						newValue.values().stream().findFirst().ifPresent(p -> {
							CurrentSaleReport.id = p.getSalesReport_Id();
							CurrentSaleReport.date = p.getSalesReport_Date();
						});
					}
				});
				...
				detailBtn.setOnAction(event -> {
					if (event.getSource() == detailBtn) {
						if (CurrentSaleReport.id == 0) {
							
							alertMessage.errorMessage("Please select the object you want to change information!");
							return;
						}
						try {
							FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/SalesReportDialog.fxml"));
